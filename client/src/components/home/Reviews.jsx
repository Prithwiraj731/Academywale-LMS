      ...
      <div className="max-w-6xl mx-auto px-4">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-3 xs:gap-4 sm:gap-8">
          {testimonials.length === 0 ? (
            ...
            const img = cld
              .image(rev.image) // Use the public_id from the database
              .format('auto')
              .quality('auto')
            ...
            return (
              <div key={rev._id} className="flex items-start bg-[#fef9f4] rounded-xl shadow-lg p-4 xs:p-5 gap-3 xs:gap-4 sm:gap-8 border border-yellow-200">
                {rev.image && <AdvancedImage cldImg={img} alt={rev.name} className="w-12 h-12 xs:w-14 xs:h-14 rounded-full object-cover border-2 border-[#d4af37]" />}
                <div>
