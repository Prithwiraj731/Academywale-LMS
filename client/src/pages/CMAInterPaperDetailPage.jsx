...
const CMAInterPaperDetailPage = () => {
  ...
  const currentPaper = papersData.cma.intermediate.find(p => `paper-${p.id}` === paperSlug);
  ...
  useEffect(() => {
    async function fetchCourses() {
      ...
      setError('');
      try {
        console.log(`Fetching CMA intermediate courses from: ${API_URL}/api/courses/CMA/intermediate/${paperId}`);
        
        // Try different URL variations
        const urlVariations = [
          `${API_URL}/api/courses/CMA/intermediate/${paperId}?includeStandalone=true`,
          `${API_URL}/api/courses/cma/intermediate/${paperId}?includeStandalone=true`,
        ...
        {currentPaper ? (
          <div className="text-center mb-8">
            <h2 className="text-3xl sm:text-4xl font-extrabold text-gray-900 mb-2 tracking-tight drop-shadow-lg">
              CMA Intermediate Paper - {currentPaper.id}
            </h2>
